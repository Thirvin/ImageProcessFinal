...
parser.add_argument("--aesop", action="store_true", default=False)
...
import yaml
with open('loss.yaml', 'r') as f:
    cfg = yaml.load(f, Loader = yaml.SafeLoader)
l_aesop = build_loss(cfg['aesop_opt']).to(device)
...
def train(epoch):
    ...
    for iteration, (lr_tensor, hr_tensor) in enumerate(training_data_loader, 1):
        ...
        loss_aesop = l_aesop(sr_tensor, hr_tensor)
        if args.aesop:
            loss_sr = loss_aesop 
        else :
            loss_sr = loss_l1
        ...
        optimizer.step()
        if iteration % 100 == 0:
            print("===> Epoch[{}]({}/{}): Loss_l1: {:.5f}, Loss_aesop {:.5f}".format(epoch, iteration, len(training_data_loader),
                                                                  loss_l1.item(), loss_aesop.item()))
